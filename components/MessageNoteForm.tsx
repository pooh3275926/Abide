      ...
      <div className="bg-beige-50 dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input type="text" name="speaker" placeholder="講員" value={formData.speaker} onChange={handleChange} className="w-full p-2 rounded border bg-white dark:bg-gray-700" />
            <input type="date" name="date" value={formData.date} onChange={handleChange} className="w-full p-2 rounded border bg-white dark:bg-gray-700" />
          </div>
          <textarea name="content" placeholder="筆記內容..." value={formData.content} onChange={handleChange} rows={8} className="w-full p-2 rounded border bg-white dark:bg-gray-700" />
          <input type="text" name="tags" placeholder="標籤 (以逗號分隔)" value={tagInput} onChange={handleTagChange} className="w-full p-2 rounded border bg-white dark:bg-gray-700" />
        </div>
        <div className="flex justify-end gap-4 mt-6">
          <button onClick={onCancel} className="px-4 py-2 rounded bg-gray-300 dark:bg-gray-600">取消</button>
          <button onClick={handleSave} className="px-4 py-2 rounded bg-gold-DEFAULT text-black dark:text-white">儲存</button>
        </div>
      </div>
    </div>
  );
